...
namespace MessageEncoder
{
    public class Encoder
    {
        ...
        */
        ...
        public void SendConsignePolaire(SerialPort port, float vitesseLineaireConsigne, float vitesseAngulaireConsigne)
        {
            byte[] linearSpeedByteArray = BitConverter.GetBytes(vitesseLineaireConsigne);
            byte[] angularSpeedByteArray = BitConverter.GetBytes(vitesseAngulaireConsigne);
            byte[] dataToSend = new byte[8];
            Buffer.BlockCopy(linearSpeedByteArray, 0, dataToSend, 0, 4);
            Buffer.BlockCopy(angularSpeedByteArray, 0, dataToSend, 4, 4);
            UartEncodeAndSendMessage(port, 0x1061, dataToSend.Length, dataToSend);
        }

        public void SendVitesseLineaireConsigne(SerialPort port, float vitesseLineaireConsigne)
        {
            byte[] floatArray = BitConverter.GetBytes(vitesseLineaireConsigne);
            UartEncodeAndSendMessage(port, 0x00AA, floatArray.Length, floatArray);
        }

        public void SendVitesseAngulaireConsigne(SerialPort port, float vitesseAngulaireConsigne)
        {
            byte[] floatArray = BitConverter.GetBytes(vitesseAngulaireConsigne);
            UartEncodeAndSendMessage(port, 0x0088, floatArray.Length, floatArray);
        }
